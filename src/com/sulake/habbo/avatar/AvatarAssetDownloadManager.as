package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_3029:String;
      
      private var var_1404:String;
      
      private var var_2191:Boolean;
      
      private var var_1139:int = 3;
      
      private var var_507:AssetLoaderStruct;
      ...
      private const const_1983:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_507 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_1138 = new Timer(this.const_2406,1);
         this.var_1138.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_507)
         {
            this.var_507.addEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_507.addEventListener(AssetLoaderEvent.const_40,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_507)
         {
            this.var_507.removeEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_507.removeEventListener(AssetLoaderEvent.const_40,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_1139 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1404);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_1139);
            if(this.var_1404.indexOf("?") > 0)
            {
               _loc2_ = this.var_1404 + "&retry=" + this.var_1139;
            }
            else
            {
               _loc2_ = this.var_1404 + "?retry=" + this.var_1139;
            ...
            this.var_507 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_2191 = true;
         for each(_loc2_ in this.var_1405)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_3029,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1403[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1403[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_2191)
         {
            this.var_1405.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_577[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_577[_loc3_] = _loc6_;
            }
            this.var_1137[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         for(_loc3_ in this.var_1137)
         {
            ...
            if(_loc7_)
            {
               ...
               delete this.var_577[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_1137[_loc3_];
         }
         while(_loc6_ < this.var_930.length)
         {
            _loc5_ = this.var_930[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_930.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_1138.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_929.length > 0 && this.var_930.length < this.const_1983)
         {
            _loc1_ = this.var_929[0];
            _loc1_.startDownloading();
            this.var_930.push(this.var_929.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_929.indexOf(param1) == -1 && this.var_930.indexOf(param1) == -1)
         {
            this.var_929.push(param1);
            this.processPending();
         }
      }
