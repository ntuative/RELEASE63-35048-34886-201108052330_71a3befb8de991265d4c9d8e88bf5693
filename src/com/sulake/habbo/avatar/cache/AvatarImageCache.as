package com.sulake.habbo.avatar.cache
{
   ...
   public class AvatarImageCache
   {
      ...
      private var var_1500:IAvatarImage;
      ...
      private var var_129:AvatarImagePartContainer;
      
      private var var_2519:Point;
      ...
      private var var_2518:ByteArray;
      
      private var var_2520:ImageData;
      
      private var var_1044:PaletteMap;
      ...
      public function setGeometryType(param1:String) : void
      {
         ...
         if(this.var_818 == GeometryType.const_1248 && param1 == GeometryType.const_1384 || this.var_818 == GeometryType.const_1384 && param1 == GeometryType.const_1248)
         {
            this.var_818 = param1;
            this.var_99 = null;
            return;
      ...
      public function getImageContainer(param1:String, param2:int) : AvatarImageBodyPartContainer
      {
            ...
            if(this.var_390.definition.isAnimation)
            {
               ...
               if(_loc12_ != null)
               {
                  ...
                  if(_loc14_ != null)
                  {
                     ...
                     else if(_loc14_.type == AnimationLayerData.const_629)
                     {
                        _loc3_ = _loc11_;
                     }
         ...
         if(_loc10_ == null)
         {
            _loc15_ = this._structure.getParts(param1,this.var_1500.getFigure(),_loc5_,this.var_818,_loc3_,_loc6_,_loc7_);
         ...
         if(this.var_505 == null)
         {
            ...
            this.var_505 = this.renderBodyPart(_loc3_,_loc16_,_loc4_,this.var_390);
            if(this.var_505 == null)
            {
               return null;
            }
      ...
      private function renderBodyPart(param1:int, param2:Array, param3:int, param4:IActiveActionData) : AvatarImageBodyPartContainer
      {
         ...
         if(!this.var_99)
         {
            this.var_99 = this._structure.getCanvas(this._scale,this.var_818);
            if(!this.var_99)
            {
               return null;
            }
            this.var_2519 = new Point(0,this.var_99.height - int(this.var_99.width / 4));
         }
         var _loc5_:int = param1;
         var _loc6_:Boolean = AvatarDirectionAngle.const_1866[param1];
         var _loc9_:String = param4.definition.assetPartDefinition;
         var _loc10_:String = param4.definition.state;
         var _loc11_:Boolean = _loc10_ == AvatarAction.const_760 || _loc10_ == AvatarAction.const_435;
         var _loc12_:int = param2.length - 1;
         _loc8_ = _loc12_;
         while(_loc8_ >= 0)
         {
            this.var_129 = param2[_loc8_] as AvatarImagePartContainer;
            if(!(param1 == 7 && (this.var_129.partType == "fc" || this.var_129.partType == "ey")))
            {
               this.var_1512 = this.var_129.bodyPartId;
               this.var_191 = this.var_129.partType;
               this.var_1841 = this.var_129.partId;
               this._frame = this.var_129.getFrameIndex(param3);
               _loc5_ = param1;
               _loc7_ = false;
               if(_loc6_)
               {
                  ...
                  else
                  {
                     ...
                     if(this.var_129.flippedPartType != this.var_191)
                     {
                        this.var_191 = this.var_129.flippedPartType;
                     }
                  }
               }
               this.var_1511 = this._scale + "_" + this.var_129.action.assetPartDefinition + "_" + this.var_191 + "_" + this.var_1841 + "_" + _loc5_ + "_" + this._frame;
               this.var_457 = this._assets.getAssetByName(this.var_1511) as BitmapDataAsset;
               if(this.var_457 == null)
               {
                  this.var_1511 = this._scale + "_std_" + this.var_191 + "_" + this.var_1841 + "_" + _loc5_ + "_0";
                  this.var_457 = this._assets.getAssetByName(this.var_1511) as BitmapDataAsset;
               }
               if(!(!this.var_457 || !this.var_457.content))
               {
                  if(this.var_129.paletteMapId > -1)
                  {
                     this.var_1044 = this._structure.renderManager.paletteManager.getPalette(this.var_129.paletteMapId);
                     if(this.var_1044 != null)
                     {
                        this.var_1009 = this.var_457.content as BitmapData;
                        this.var_192 = new BitmapData(this.var_1009.width,this.var_1009.height);
                        this.var_2518 = this.var_1044.colorize(this.var_1009.getPixels(this.var_1009.rect));
                        this.var_192.setPixels(this.var_192.rect,this.var_2518);
                     }
                     else
                     {
                        this.var_192 = (this.var_457.content as BitmapData).clone();
                     }
                  }
                  else
                  {
                  ...
                  if(_loc11_ && (this.var_1512 == AvatarBodyPartType.const_299 || this.var_1512 == AvatarBodyPartType.const_1794 || this.var_1512 == AvatarBodyPartType.const_1833) && this.var_129.color != null)
                  {
                     _loc16_ = 0.7;
                     _loc17_ = this.var_129.color.colorTransform;
                  ...
                  else if(this.var_129.isColorable && this.var_129.color != null)
                  {
                     this.var_192.colorTransform(this.var_192.rect,this.var_129.color.colorTransform);
                  }
                  if(this.var_129.isBlendable)
                  {
                     this.var_192.colorTransform(this.var_192.rect,this.var_129.blendTransform);
                  }
                  this.var_1010 = this.var_457.offset.clone();
                  if(_loc7_)
                  {
                     if(this._scale == AvatarScaleType.const_61)
                     {
                  ...
                  this.var_2520 = new ImageData(this.var_192,this.var_1010,_loc7_);
                  this.var_1011.push(this.var_2520);
               }
            }
            _loc8_--;
         }
         ...
         var _loc13_:ImageData = this.createUnionImage(this.var_1011,_loc6_);
         var _loc14_:Point = new Point(-1 * _loc13_.regpoint.x,this.var_2519.y - _loc13_.regpoint.y);
         if(_loc6_)
         {
            if(this._scale == AvatarScaleType.const_61)
            {
         ...
         _loc8_ = this.var_1011.length - 1;
         while(_loc8_ >= 0)
         {
            _loc15_ = this.var_1011.pop();
            if(_loc15_)
            {
               _loc15_.dispose();
            }
