package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_854:IFigurePartSet;
      ...
      private var var_474:IAvatarRenderManager;
      
      private var var_1172:Boolean;
      
      private var var_1954:BitmapData;
      
      private const const_2390:int = 13421772;
      
      private const const_2391:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this._colors = param4;
         this.var_1955 = param5;
         if(param3 == null)
         {
            this.var_770 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1956 = Math.max(this.var_1956,_loc6_.colorLayerIndex);
            }
         }
         this.var_474 = this.var_78.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_770 != null && !this.var_1955)
            {
               _loc3_ = this.var_770;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_854)
            {
               switch(this.var_854.clubLevel)
               {
                  case HabboClubLevelEnum.const_53:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_33:
                     _loc2_.style = HabboIconType.const_911;
                     break;
                  case HabboClubLevelEnum.const_35:
                     _loc2_.style = HabboIconType.const_761;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1579.color = this.const_2390;
         }
         else
         {
            this.var_1579.color = this.const_2391;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_474.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_474.isFigureReady(_loc1_))
         {
            this.var_474.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_390[_loc6_] + "_" + FigureData.const_197;
               _loc4_ = this.var_474.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_680 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1954)
               {
                  ...
                  this.var_1954 = _loc9_.content as BitmapData;
               }
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               if(this.var_1955 && _loc2_.colorLayerIndex > 0)
               {
