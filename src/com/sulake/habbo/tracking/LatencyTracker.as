package com.sulake.habbo.tracking
{
   ...
   public class LatencyTracker
   {
      ...
      public function init() : void
      {
         ...
         this.var_2027 = int(this._configuration.getKey("latencytest.interval"));
         this.var_2025 = int(this._configuration.getKey("latencytest.report.index"));
         this.var_2766 = int(this._configuration.getKey("latencytest.report.delta"));
      ...
      public function update(param1:uint, param2:int) : void
      {
         ...
         if(param2 - this.var_2024 > this.var_2027)
         {
            this.testLatency();
         }
      }
      ...
      private function testLatency() : void
      {
         ...
         this.var_696.add(this.var_1619,this.var_2024);
         this._connection.send(new LatencyPingRequestMessageComposer(this.var_1619));
         ++this.var_1619;
      }
      ...
      private function onPingResponse(param1:IMessageEvent) : void
      {
         ...
         var _loc10_:int = 0;
         var _loc11_:* = null;
         if(this.var_696 == null || this.var_260 == null)
         {
            return;
         }
         var _loc2_:LatencyPingResponseMessageParser = (param1 as LatencyPingResponseMessageEvent).getParser();
         var _loc3_:int = this.var_696.getValue(_loc2_.requestId);
         this.var_696.remove(_loc2_.requestId);
         var _loc4_:int = getTimer() - _loc3_;
         this.var_260.push(_loc4_);
         if(this.var_260.length == this.var_2025 && this.var_2025 > 0)
         {
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            ...
            if(Math.abs(_loc9_ - this.var_2026) > this.var_2766 || this.var_2026 == 0)
            {
