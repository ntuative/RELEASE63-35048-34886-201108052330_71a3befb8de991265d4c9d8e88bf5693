package com.sulake.habbo.toolbar.extensions
{
   ...
   public class ClubDiscountPromoExtension
   {
      ...
      private static const const_1478:int = 13;
      
      private static const const_1061:int = 14;
      ...
      private function createWindow(param1:IAssetLibrary, param2:IHabboWindowManager) : IWindowContainer
      {
         ...
         if(_loc4_)
         {
            ...
            if(_loc3_)
            {
               this.var_82 = _loc3_.findChildByName("flashing_animation") as IBitmapWrapperWindow;
               if(this.var_82)
               {
                  _loc5_ = this._assets.getAssetByName("extend_hilite_png") as IAsset;
                  if(_loc5_)
                  {
                     this.var_788 = _loc5_.content as BitmapData;
                     if(this.var_788)
                     {
                        this.var_82.bitmap = this.var_788.clone();
                     }
                  }
      ...
      private function animate(param1:Boolean) : void
      {
         if(param1)
         {
            ...
            this.var_633 = new Timer(15000);
            this.var_633.addEventListener(TimerEvent.TIMER,this.onTriggerTimer);
            this.var_633.start();
         }
         else
         {
      ...
      private function resetAnimationVariables() : void
      {
         ...
         this.var_82.y = 3;
         this.var_82.bitmap = this.var_788.clone();
         this.var_82.height = this._view.height - 6;
         this.var_82.width = this.var_82.bitmap.width;
         this.var_82.invalidate();
         this.var_1788 = this._view.width - 7 - this.var_82.bitmap.width;
         this.var_1477 = 0;
      }
      
      private function startAnimationTimer() : void
      {
         this.var_640 = new Timer(25,26);
      ...
      private function onAnimationTimer(param1:TimerEvent) : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         this.var_82.x = 3 + this.var_1477 / 20 * this.var_1788;
         if(this.var_82.x > this.var_1788)
         {
            _loc2_ = this._view.width - 4 - this.var_82.x;
            _loc3_ = new BitmapData(_loc2_,this.var_788.height);
            _loc3_.copyPixels(this.var_788,new Rectangle(0,0,_loc2_,this.var_788.height),new Point(0,0));
            this.var_82.bitmap = _loc3_;
            this.var_82.width = _loc2_;
         }
         this.var_82.invalidate();
         ++this.var_1477;
      }
