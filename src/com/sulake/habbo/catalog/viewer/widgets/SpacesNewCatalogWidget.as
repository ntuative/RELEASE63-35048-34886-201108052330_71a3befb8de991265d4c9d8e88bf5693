package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class SpacesNewCatalogWidget extends ItemGridCatalogWidget implements IDisposable, ICatalogWidget, IItemGrid
   {
      ...
      private var var_479:ISelectorWindow;
      ...
      public function SpacesNewCatalogWidget(param1:IWindowContainer, param2:ISessionDataManager)
      {
         ...
         super(param1,param2);
         this.var_479 = _window.findChildByName("groups") as ISelectorWindow;
         if(this.var_479)
         {
            _loc3_ = 0;
            while(_loc3_ < this.var_479.numSelectables)
            {
               _loc4_ = this.var_479.getSelectableAt(_loc3_);
      ...
      override public function init() : Boolean
      {
         ...
         this.switchCategory(this.var_118[this.var_565]);
      ...
      {
         var _loc2_:int = this.var_690[this.var_565];
         var _loc3_:Offer = this.var_223[this.var_565][_loc2_];
      ...
      {
         var _loc2_:* = null;
         if(param1 > -1 && param1 < var_515.numGridItems)
         {
            _loc2_ = this.var_223[this.var_565][param1];
      ...
      override public function select(param1:IGridItem) : void
      {
         ...
         this.var_690[this.var_565] = (this.var_223[this.var_565] as Array).indexOf(_loc2_);
         this.updateRoomPreview();
      }
      
      private function updateRoomPreview() : void
      {
         var _loc1_:int = this.var_690[0];
         var _loc2_:int = this.var_690[1];
         var _loc3_:int = this.var_690[2];
         var _loc4_:Offer = this.var_223[0][_loc1_];
         var _loc5_:Offer = this.var_223[1][_loc2_];
         var _loc6_:Offer = this.var_223[2][_loc3_];
      ...
      private function createOfferGroups() : Boolean
      {
         ...
         {
            if(!(_loc1_.pricingModel == Offer.const_426 || _loc1_.pricingModel == Offer.const_436))
            {
               continue;
            }
            ...
            while(this.var_223.length < this._groupNames.length)
            {
               this.var_223.push(new Array());
            }
            switch(_loc4_)
            {
               case "floor":
                  (this.var_223[_loc5_] as Array).push(_loc1_);
                  break;
               case "wallpaper":
                  (this.var_223[_loc5_] as Array).push(_loc1_);
                  break;
      ...
      private function onSelectGroup(param1:WindowEvent) : void
      {
         ...
         if(_loc2_)
         {
            _loc3_ = this.var_479.getSelectableIndex(_loc2_);
      ...
      private function switchCategory(param1:String) : void
      {
         ...
         this.var_479.setSelected(this.var_479.getSelectableByName(param1));
         var _loc2_:int = -1;
         switch(param1)
         {
            case "group.walls":
         ...
         if(_loc2_ > -1)
         {
            ...
            if(var_515)
            {
               var_515.destroyGridItems();
            }
            page.replaceOffers(this.var_223[this.var_565],false);
            resetTimer();
            populateItemGrid();
            ...
            _loc3_ = this.var_690[this.var_565];
