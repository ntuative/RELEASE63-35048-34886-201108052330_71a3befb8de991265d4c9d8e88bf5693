package com.sulake.habbo.sound.music
{
   ...
   public class HabboMusicController implements IHabboMusicController, IDisposable
   {
      ...
      private static const const_1521:int = HabboMusicPrioritiesEnum.const_125;
      ...
      public function HabboMusicController(param1:HabboSoundManagerFlash10, param2:IEventDispatcher, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_823.push(new TraxSongInfoMessageEvent(this.onSongInfoMessage));
         this.var_823.push(new UserSongDisksInventoryMessageEvent(this.onSongDiskInventoryMessage));
         for each(_loc5_ in this.var_823)
         {
            this._connection.addMessageEvent(_loc5_);
         }
         this.var_93.addEventListener(RoomEngineSoundMachineEvent.const_797,this.onJukeboxInit);
         this.var_93.addEventListener(RoomEngineSoundMachineEvent.const_290,this.onJukeboxDispose);
         this.var_93.addEventListener(RoomEngineSoundMachineEvent.const_946,this.onSoundMachineInit);
         this.var_93.addEventListener(RoomEngineSoundMachineEvent.const_810,this.onSoundMachineDispose);
         this.var_1228 = new Timer(1000);
         this.var_1228.start();
         this.var_1228.addEventListener(TimerEvent.TIMER,this.sendNextSongRequestMessage);
         ...
         {
            this.var_326[_loc6_] = null;
            this.var_805[_loc6_] = 0;
            _loc6_++;
         }
      }
      ...
      protected function onSongFinishedPlayingEvent(param1:SoundCompleteEvent) : void
      {
         ...
         {
            if(this.getTopRequestPriority() == this._priorityPlaying && this.getSongRequestCountAtPriority(this._priorityPlaying) == this.var_1826)
            {
               this.resetSongStartRequest(this._priorityPlaying);
            }
            _loc2_ = this._priorityPlaying;
            this.playSongWithHighestPriority();
            if(_loc2_ >= HabboMusicPrioritiesEnum.const_462)
            {
      ...
      public function dispose() : void
      {
         ...
         if(!this._disposed)
         {
            ...
            if(this._connection)
            {
               for each(_loc1_ in this.var_823)
               {
            ...
            if(this.var_93)
            {
               this.var_93.removeEventListener(RoomEngineSoundMachineEvent.const_797,this.onJukeboxInit);
               this.var_93.removeEventListener(RoomEngineSoundMachineEvent.const_290,this.onJukeboxDispose);
               this.var_93.removeEventListener(RoomEngineSoundMachineEvent.const_946,this.onSoundMachineInit);
               this.var_93.removeEventListener(RoomEngineSoundMachineEvent.const_810,this.onSoundMachineDispose);
            }
      ...
      private function addSongStartRequest(param1:int, param2:int, param3:Number, param4:Number, param5:Number, param6:Number) : Boolean
      {
         ...
         this.var_326[param1] = _loc7_;
         this.var_805[param1] += 1;
         return true;
      }
      
      private function getSongStartRequest(param1:int) : SongStartRequestData
      {
         return this.var_326[param1];
      }
      ...
      private function getSongIdRequestedAtPriority(param1:int) : int
      {
         ...
         if(this.var_326[param1] == null)
         {
            return -1;
         }
         var _loc2_:SongStartRequestData = this.var_326[param1];
      ...
      private function getSongRequestCountAtPriority(param1:int) : int
      {
         ...
         return this.var_805[param1];
      }
      
      private function getTopRequestPriority() : int
      {
         var _loc1_:int = this.var_326.length - 1;
         while(_loc1_ >= 0)
         {
            if(this.var_326[_loc1_] != null)
            {
               return _loc1_;
            }
            _loc1_--;
      ...
      public function getSongDiskInventoryDiskId(param1:int) : int
      {
         ...
         {
            return this.var_383.getKey(param1);
         }
         return -1;
      }
      ...
      public function getSongDiskInventorySongId(param1:int) : int
      {
         if(param1 >= 0 && param1 < this.var_383.length)
         {
            return this.var_383.getWithIndex(param1);
         }
         return -1;
      }
      ...
      public function samplesUnloaded(param1:Array) : void
      {
         ...
         while(_loc2_ < this.var_286.length)
         {
            ...
            if(_loc3_.id != this.var_1498 && _loc4_ != null && _loc4_.ready)
            {
      ...
      public function get samplesIdsInUse() : Array
      {
         ...
         {
            if(this.var_326[_loc2_] != null)
            {
               _loc3_ = this.var_326[_loc2_];
      ...
      private function sendNextSongRequestMessage(param1:TimerEvent) : void
      {
         ...
         this._connection.send(new GetSongInfoMessageComposer(this.var_807));
         Logger.log("Requested song info\'s : " + this.var_807);
         this.var_807 = new Array();
      }
      ...
      private function onSongInfoMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc5_ < _loc4_.length)
         {
            ...
            if(_loc7_)
            {
               ...
               while(this.var_806.indexOf(_loc6_.id) != -1)
               {
                  this.var_806.splice(this.var_806.indexOf(_loc6_.id),1);
      ...
      private function playSongObject(param1:int, param2:int) : Boolean
      {
         ...
         this.var_1826 = this.getSongRequestCountAtPriority(param1);
         this.var_1498 = param2;
         if(this._priorityPlaying <= const_1521)
         {
            this.notifySongPlaying(_loc4_);
         }
         if(param1 > HabboMusicPrioritiesEnum.const_125)
         {
            this._events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_129,param1,_loc4_.id,-1));
         }
      ...
      private function notifySongPlaying(param1:SongDataEntry) : void
      {
         ...
         if(param1.length >= 8000 && (this.var_2464 != param1.id || _loc3_ > this.var_2465 + 8000))
         {
            this._soundManager.notifyPlayedSong(param1.name,param1.creator);
            this.var_2464 = param1.id;
            this.var_2465 = _loc3_;
         }
      }
      ...
      private function onSoundMachineInit(param1:Event) : void
      {
         ...
         this.var_541 = new SoundMachinePlayListController(this._soundManager,this,this._events,this.var_93,this._connection) as IPlayListController;
      }
