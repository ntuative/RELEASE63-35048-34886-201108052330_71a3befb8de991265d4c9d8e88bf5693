package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1460:String = "asset-libraries";
      
      private static const const_1458:String = "library";
      
      private static const const_1461:String = "service-libraries";
      
      private static const const_1457:String = "library";
      
      private static const const_1465:String = "component-libraries";
      
      private static const const_1459:String = "library";
      
      private static const const_1463:String = "error_data";
      
      private static const const_1462:String = "error_cat";
      
      private static const const_1464:String = "error_desc";
       
      
      private var var_372:LibraryLoaderQueue;
      
      private var var_778:IEventDispatcher;
      
      private var var_529:uint;
      
      private var var_371:Function;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         this.var_629 = new Array();
         var_440 = param1;
         var _loc3_:int = 0;
         while(_loc3_ < CoreComponentContext.const_107)
         {
            this.var_273.push(new Array());
            this.var_629.push(0);
            _loc3_++;
         }
         ...
         switch(param2 & 0)
         {
            case Core.const_1379:
               debug("Core; using simple frame update handler");
               this.var_371 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_862:
               debug("Core; using complex frame update handler");
               this.var_371 = this.complexFrameUpdateHandler;
               break;
            case Core.const_850:
               debug("Core; using profiler frame update handler");
               this.var_371 = this.profilerFrameUpdateHandler;
               this.var_214 = new Profiler(this);
               attachComponent(this.var_214,[new IIDProfiler()]);
               var_440.addChild(new ProfilerViewer(this.var_214));
               break;
            case Core.const_575:
               debug("Core; using experimental frame update handler");
               this.var_371 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_371 = this.debugFrameUpdateHandler;
         }
      }
      ...
      {
         return this.var_529 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_273[i] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1463,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1462,String(param3));
         ErrorReportStorage.setParameter(const_1464,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         if(this.var_372 == null)
         {
            this.var_372 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1460)[0];
            if(node != null)
            {
               list = node.child(const_1458);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_227,e);
         }
         try
         {
            node = config.child(const_1461)[0];
            if(node != null)
            {
               list = node.child(const_1457);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_227,e);
         }
         try
         {
            node = config.child(const_1465)[0];
            if(node != null)
            {
               list = node.child(const_1459);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_227,e);
         }
         if(!disposed)
         {
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_778)
                  {
                     this.var_778.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_529 - this.var_372.length,this.var_529,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_214 ? 0 : this.var_1461 & 0;
         if(_loc3_ == Core.const_575)
         {
            this.var_273[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_273[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc4_:int = !!this.var_214 ? 0 : this.var_1461 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_107)
         {
            _loc3_ = this.var_273[_loc5_] as Array;
            if(_loc4_ == Core.const_575)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_371(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_629[priority] = 0;
            receivers = this.var_273[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_107)
         {
            ...
            {
               if(this.var_629[priority] < priority)
               {
                  ++this.var_629[priority];
                  skip = true;
               }
            }
            if(!skip)
